...
Requires an access token (CTDF_MASTODON_TOKEN) and instance URL
(CTDF_MASTODON_INSTANCE, e.g. https://mastodon.social).
...
class MastodonPlatform(SocialPlatform):
    ...
    name = "mastodon"
    env_vars = ["CTDF_MASTODON_INSTANCE", "CTDF_MASTODON_TOKEN"]
    max_length = 500
    ...
    def post(self, message: str) -> dict[str, Any]:
        ...
        if not self.is_configured():
            return {
                "success": False,
                "platform": self.name,
                "error": "Missing credentials. Set CTDF_MASTODON_INSTANCE and CTDF_MASTODON_TOKEN.",
            }
