...
def locked_config_read(config_path: str | Path) -> dict:
    """Read a JSON config file under a shared lock.
...
def locked_config_write(
    ...
    *,
    ...
    for attempt in range(max_retries):
        try:
            with _config_file_lock(lock_path, timeout=timeout):
                ...
                # for atomicity. Using the same directory ensures same
                # filesystem (required for os.replace).
    ...
    if last_error:
        raise ConfigLockError(
            f"Failed to write {config_path} after {max_retries} attempts: "
            f"{last_error}"
        )
    return False
...
def locked_config_update(
    ...
    *,
    ...
    if last_error:
        raise ConfigLockError(
            f"Failed to update {config_path} after {max_retries} attempts: "
            f"{last_error}"
        )
    return False
