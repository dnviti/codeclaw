...
```bash
gh issue list --repo "$TRACKER_REPO" --label task --state all --limit 500 --json title --jq '.[].title' | python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py next-id --type task --source platform-titles
```
...
`python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py next-id --type task`

Section headers in to-do.txt:
`python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py sections --file to-do.txt`
...
**Local / dual sync mode:**
Run: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py duplicates --keywords "keyword1,keyword2,keyword3" --files "to-do.txt,progressing.txt,done.txt"`
