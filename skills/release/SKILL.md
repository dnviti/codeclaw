...
>
> Their changes may be on the release branch. Consider running `/test-review` to complete testing before releasing."
...
**1c. Check release plan (if available):**

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/release_manager.py release-plan-next
```

If a next release exists in `releases.json`:
1. Check if all its tasks are done (cross-reference task statuses).
2. If tasks are incomplete, warn the user:

   > "**Release plan note:** Release **vX.Y.Z** (*theme*) has N incomplete tasks:
   > - TASK-CODE — Title (status)
   > - ..."

   Use `AskUserQuestion` with options:
   - **"Release with incomplete tasks"** — proceed anyway
   - **"Abort and complete tasks first"** — stop here

   STOP HERE after calling `AskUserQuestion`. Do NOT proceed until the user responds.

3. If all tasks are done, note the planned version for use in Step 4.

If no `releases.json` exists or no planned releases, skip this check silently.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/release_manager.py parse-commits --since "$LAST_TAG"
```
...
**If a release plan version was identified in Step 1c**, present it as the suggested version instead of auto-detecting from commits. Still allow the user to override. Fall back to commit-based detection if no release plan exists.

Run the bump calculator:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/release_manager.py suggest-bump --current-version "$CURRENT_VERSION" --suggested-bump "$SUGGESTED_BUMP"
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/release_manager.py parse-commits --since "$LAST_TAG" | python3 ${CLAUDE_PLUGIN_ROOT}/scripts/release_manager.py generate-changelog --version "$NEW_VERSION" --date "$(date +%Y-%m-%d)"
```
...
### Added
- Feature description (TASK-0001)
- Another feature

### Fixed
- Bug fix description (TASK-0002)
```
...
```
## Release vX.Y.Z — [theme]

### Tasks Included
- TASK-CODE — Title (#issue)
- TASK-CODE — Title (#issue)
...
1. Collect task codes from commits between main and the release branch:
   ```bash
   TASK_CODES=$(git log main..<RELEASE_BRANCH> --oneline --no-merges | grep -oE '[A-Z]{3,5}-[0-9]{4}' | sort -u)
   ```
...
3. Build enriched release notes:
   ...
   ## Issues Resolved
   - #N1 — [PREFIX-XXXX] Task title
   - #N2 — [PREFIX-XXXX] Task title
...
If a release plan was active for this version (identified in Step 1c), mark the release as released:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/release_manager.py release-plan-mark-released --version X.Y.Z
```

If the command fails or no release plan exists, skip silently.
...
4. **Present suggested groupings** with reasoning:
   ...
   - MKT-0004 — Add plugin search
   - MKT-0005 — Plugin ratings system
   - UIX-0006 — Plugin detail page redesign

   **Reasoning:** Same MKT- prefix (marketplace domain), MKT-0005 depends on MKT-0004,
   UIX-0006 modifies the same plugin UI components.
   ```
...
3. For each release (ordered by version), show:
   ...
   **Status:** planned | **Target:** 2026-04-01 | **Progress:** 1/3 (33%)
   ...
   |----------|--------------------------|--------------|----------|
   | MKT-0004 | Add plugin search        | [~] progress | HIGH     |
   | MKT-0005 | Plugin ratings system    | [ ] todo     | MEDIUM   |
   | UIX-0006 | Plugin detail redesign   | [x] done     | LOW      |
   ```
