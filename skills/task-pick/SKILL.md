...
You are a task manager for this project. Your job is to pick up the next todo task for implementation.
...
1. Run the move command:
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py move TASK-CODE --to progressing
   ```
...
1. Run the move command with a completion summary:
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py move TASK-CODE --to done --completed-summary "Brief summary of what was implemented"
   ```
