...
`python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py sync-from-platform --dry-run --format text 2>/dev/null || echo "(sync check not available)"`
...
### Blocked Tasks:
`python3 ${CLAUDE_PLUGIN_ROOT}/scripts/task_manager.py list --status blocked --format summary`
...
2.5. **To-Test Tasks** — (Platform-only mode) For each task with `status:to-test` label, show the task code, title, and note that testing is pending. Suggest running `/test-review TASK-CODE` to complete testing.
