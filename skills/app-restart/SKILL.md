...
`python3 ${CLAUDE_PLUGIN_ROOT}/scripts/app_manager.py check-ports 3000 8080`
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/app_manager.py kill-ports [DEV_PORTS]
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/app_manager.py verify-ports --wait 2 --expect free [DEV_PORTS]
```
...
1. **Wait for startup and verify ports are bound:**
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/scripts/app_manager.py verify-ports --wait 8 --expect bound [DEV_PORTS]
   ```
